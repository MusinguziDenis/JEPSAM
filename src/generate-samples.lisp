...
(defun setup-generation ()
 ...
 ;;                  (make-pose "map" (generate-random-location))))
  ...
  (setf (gethash #'*on-transformation* *transformation-hash*) " #'*on-transformation* ")
  ...
  (if (> (random 1.0) *move-command-fraction*)
      (progn
        ...
        (if (eq *transform* #'*on-transformation*)
            ...
            (progn
              (print "entering ontop transformation condition")
              (setf *obj2-type* (random-choice *ontop-objects*))))
              ;;(setf *obj2-location* (random-choice *obj1-poses*))))
        ;; if both objects are same type and have same color
        (if (and (equal *obj1-type* *obj2-type*) (equal *obj1-color* *obj2-color*))
            ;; then pick a different color for the second object
            (progn
              (print "entering the same object type and color condition")
              (let* ((temp (remove *obj1-color* *available-colors*)))
              (setf *obj2-color* (random-choice temp)))))
